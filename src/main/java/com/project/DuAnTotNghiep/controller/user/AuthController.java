...
public class AuthController {
    ...
    @Value("${app.dev.show-verification-code:false}")
    private boolean showVerificationInDev;

    private final CustomerRepository customerRepository;

    public AuthController(AccountService accountService, AccountRepository accountRepository,
            PasswordEncoder passwordEncoder, SessionService sessionService, CookieService cookieService,
            VerificationCodeService verificationCodeService, CustomerRepository customerRepository) {
...
<<<<<<< HEAD
    ...
    public String viewResetPassPage(@RequestParam String email, RedirectAttributes redirectAttributes) {
        try {
            com.project.DuAnTotNghiep.entity.VerificationCode vc = verificationCodeService
                    .createVerificationCode(email);
            if (showVerificationInDev && vc != null) {
                // Expose the code in a flash attribute for local development/testing only
                redirectAttributes.addFlashAttribute("devVerificationCode", vc.getCode());
            }
        } catch (ShopApiException exception) {
            redirectAttributes.addFlashAttribute("errorMessage", exception.getMessage());
            return "redirect:/forgot-pass";
        } catch (javax.mail.MessagingException mex) {
            // Email sending failed (SMTP/auth issue). Provide a helpful message and log the
            // exception.
            mex.printStackTrace();
            redirectAttributes.addFlashAttribute("errorMessage",
                    "Không thể gửi email xác nhận. Vui lòng kiểm tra cấu hình email (MAIL_USERNAME / MAIL_PASSWORD) hoặc liên hệ quản trị viên.");
            // Optionally, you could still redirect to reset page and let admin provide
            // alternative flow.
            return "redirect:/forgot-pass";
        }
